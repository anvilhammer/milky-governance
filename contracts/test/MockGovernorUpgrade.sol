...
contract MockGovernorUpgrade is MilkyGovernorDelegateStorageV2, MilkyGovernorEvents {

    /// @notice The name of this contract
    string public constant name = "Mock Governor Upgrade";

    /// @notice The minimum setable proposal threshold
    uint public constant MIN_PROPOSAL_THRESHOLD = 600000e18; // 600,000 CREAMY

    /// @notice The maximum setable proposal threshold
    uint public constant MAX_PROPOSAL_THRESHOLD = 2000000e18; // 2,000,000 CREAMY

    /// @notice The minimum setable voting period
    uint public constant MIN_VOTING_PERIOD = 21600; // About 24 hours

    /// @notice The max setable voting period
    uint public constant MAX_VOTING_PERIOD = 282240; // About 2 weeks

    /// @notice The min setable voting delay
    uint public constant MIN_VOTING_DELAY = 21600; // About 24 hours

    /// @notice The max setable voting delay
    uint public constant MAX_VOTING_DELAY = 216000; // About 10 days

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    uint public constant quorumVotes = 500000e18; // 500,000 CREAMY
    ...
    function initialize(address timelock_, address creamy_, uint votingPeriod_, uint votingDelay_, uint proposalThreshold_) public {
        require(address(timelock) == address(0), "MilkyGovernor::initialize: can only initialize once");
        require(msg.sender == admin, "MilkyGovernor::initialize: admin only");
        require(timelock_ != address(0), "MilkyGovernor::initialize: invalid timelock address");
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(targets.length == values.length && targets.length == signatures.length && targets.length == calldatas.length, "MilkyGovernor::propose: proposal function information arity mismatch");
        require(targets.length != 0, "MilkyGovernor::propose: must provide actions");
        require(targets.length <= proposalMaxOperations, "MilkyGovernor::propose: too many actions");
    ...
    function queue(uint proposalId) external {
        require(state(proposalId) == ProposalState.Succeeded, "MilkyGovernor::queue: proposal can only be queued if it is succeeded");
    ...
    function queueOrRevertInternal(address target, uint value, string memory signature, bytes memory data, uint eta) internal {
        require(!timelock.queuedTransactions(keccak256(abi.encode(target, value, signature, data, eta))), "MilkyGovernor::queueOrRevertInternal: identical proposal action already queued at eta");
    ...
    function castVoteBySig(uint proposalId, uint8 support, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(signatory != address(0), "MilkyGovernor::castVoteBySig: invalid signature");
    ...
    /**
      ...
      * @param oldGovernor The address for the Governor to continue the proposal id count from
      */
    function _initiate(address oldGovernor) external {
        ...
        proposalCount = MilkyGovernorDelegateStorageV2(oldGovernor).proposalCount();
